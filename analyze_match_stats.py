...
def analyze_match_stats(match_details, teams):
    ...
    for team_name, players in team_stats.items():
        for player_name, stats in players.items():
            ...
            stats["average_kills"] = stats["total_kills"] / stats["total_games"] if stats["total_games"] > 0 else 0
            stats["average_deaths"] = stats["total_deaths"] / stats["total_games"] if stats["total_games"] > 0 else 0
            stats["average_assists"] = stats["total_assists"] / stats["total_games"] if stats["total_games"] > 0 else 0
            stats["average_cs_per_game"] = stats["total_cs"] / stats["total_games"] if stats["total_games"] > 0 else 0
            stats["average_vision_score"] = stats["total_vision_score"] / stats["total_games"] if stats["total_games"] > 0 else 0
            stats["average_cs_per_minute"] = stats["total_cs"] / stats["total_game_duration"] if stats["total_game_duration"] > 0 else 0
...
if __name__ == "__main__":
    ...
    teams_with_puuid_path = os.path.join(current_dir, "teams_with_puuid.json")
